...
async function distOverClasses(values){
	...
	const labeldiseases = ['Atelectasis',
		'Consolidation',
		'Infiltration',
		'Pneumothorax',
		'Edema',
	...
	for (let i = 0; i < values.length; i++) {
		MODEL_CONFIG.LABELS[i] = labeldiseases[i];
	}

	for (let i = 0; i < values.length; i++) {
		console.log(MODEL_CONFIG.LABELS[i]);
...
function showProbResults(thispred) {
	...
	for (let i = -1; i < classes.length; i++) {
		...
		if (i == -1){
			...
			probsElement.style.borderRadius="8px";
			probsElement.style.borderColor = "white";
			probsElement.style.borderWidth="5";
		}
